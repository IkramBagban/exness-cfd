...
const Instruments = ({ selectedSymbol, prices, setSelectedSymbol }: InstrumentsProps) => {
  ...
  return (
    <div className="h-full bg-[#1E222D] flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        <div className="text-xs text-[#848E9C] mb-2 px-4 mt-2 font-medium">{debouncedSearch ? `Found ${filteredInstruments.length} results` : 'Favorites'}</div>
        <div className="space-y-1 px-2 pb-3">
          {filteredInstruments.length === 0 ? (
            <div className="text-center text-gray-500 py-4 text-sm">No instruments found</div>
          ) : (
              ...
              return (
                <div
                  ...
                  className={`flex items-center justify-between p-3 rounded cursor-pointer transition-colors ${
                    isSelected ? 'bg-blue-600/20 border border-blue-500/30' : 'hover:bg-gray-700/50 border border-transparent'
                  }`}
                >
                  <div className="flex items-center space-x-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-gray-400 truncate">{instrument.name}</div>
                    </div>
                  </div>
                  <div className="text-right flex gap-3 sm:gap-4">
                    <div className="text-xs sm:text-sm font-mono text-green-400">{currentBid.toFixed(5)}</div>
                    <div className="text-xs sm:text-sm font-mono text-red-500">{currentAsk.toFixed(5)}</div>
                  </div>
                </div>
              );
            })
